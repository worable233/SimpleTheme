...
export async function apiLogin(log: string, pwd: string, rememberme = false) {
  const { data } = await apiClient.post(buildRestUrl('simple-theme/v1/auth/login'), {
    log,
    pwd,
    rememberme,
  })
  return data as {
    success: boolean
    user?: { id: number; display_name: string; avatar: string; email: string; url?: string }
    rest_nonce?: string
    redirect_to?: string
    logout_url?: string
    code?: string
...
export async function apiRegister(user_login: string, user_email: string) {
  const { data } = await apiClient.post(buildRestUrl('simple-theme/v1/auth/register'), {
    user_login,
    user_email,
  })
  return data as {
...
export async function apiLostPassword(user_login: string) {
  const { data } = await apiClient.post(buildRestUrl('simple-theme/v1/auth/lost-password'), {
    user_login,
  })
  return data as {
    success: boolean
...
export async function apiResetPassword(key: string, login: string, pass1: string, pass2: string) {
  const { data } = await apiClient.post(buildRestUrl('simple-theme/v1/auth/reset-password'), {
    key,
    login,
    pass1,
    pass2,
