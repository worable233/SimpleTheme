...
export default defineConfig(({ command, mode }) => {
  ...
  const WP_URL = env.VITE_WP_SITE_URL || env.WP_URL || process.env.WP_URL || 'http://localhost'

  return {
    base: command === 'serve' ? '/wp-content/themes/simple-theme/dist/' : './',
    plugins: [
      vue({
        template: {
          compilerOptions: {
        ...
        workbox: {
          ...
          runtimeCaching: [
            ...
            },
            {
              ...
              urlPattern:
                /\/wp-json\/simple-theme\/v1\/(?:collection|navigation(?:\/[^/?]+)?|about|links|resolve-url)(?:[/?]|$)/,
